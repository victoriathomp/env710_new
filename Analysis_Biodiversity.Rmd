...
model1 <- lm(Abundance_Estimate ~ CANOPY_COVER + WETTED_WIDTH, data = full_salamander_data)
summary(model1)
plot(model1)
AIC(model1)
...
model2 <- lm(Abundance_Estimate ~ WOOD_LENGTH*REACH_TYPE + DEPTH_MAX*REACH_TYPE + factor(SITE), data = full_salamander_data)
summary(model2)
plot(model2)
AIC(model2)

#removing factor makes Q-Q more linear
model2B <- lm(Abundance_Estimate ~ WOOD_LENGTH*REACH_TYPE + DEPTH_MAX*REACH_TYPE, data = full_salamander_data)
summary(model2B)
plot(model2B)
AIC(model2B)
...
model2C <- lm(Abundance_Estimate ~ WOOD_LENGTH*REACH_TYPE + DEPTH_MAX*REACH_TYPE, data = full_salamander_data_no_COOK )
summary(model2C)
plot(model2C)
AIC(model2C)

model2D <- lm(Abundance_Estimate ~ WOOD_LENGTH*REACH_TYPE + DEPTH_MAX*REACH_TYPE + factor(SITE), data = full_salamander_data_no_COOK)
summary(model2D)
plot(model2D)
AIC(model2D)
```
